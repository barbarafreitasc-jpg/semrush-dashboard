...
function saveManual(data) {
  ...
  localStorage.setItem(LS_KEY, JSON.stringify({ ...data, syncedAt: new Date().toISOString() }));
}
...
export default function AIVisibilityCard({ data = null, loading = false }) {
  ...
  const save = () => {
    saveManual(draft);
    setLlm({ ...draft, syncedAt: new Date().toISOString() });
    setEditing(false);
  };

  if (loading) return (
    <div className="bg-surface-card border border-surface-border rounded-2xl p-5 animate-pulse h-52">
      <div className="h-5 bg-surface-border rounded w-48 mb-4" /><div className="h-full bg-surface-border rounded" />
    </div>
  );
  ...
  const scoreColor = llmScore >= 60 ? 'text-emerald-400' : llmScore >= 30 ? 'text-yellow-400' : llmScore > 0 ? 'text-orange-400' : 'text-slate-500';
  const llmBars = llm ? LLM_FIELDS.map(f => ({ ...f, v: llm[f.k] })).filter(f => f.v && Number(f.v) > 0) : [];
    ...
    <div className="bg-surface-card border border-purple-500/20 rounded-2xl p-5">
      ...
      {editing && (
        <div className="mb-4 bg-surface-border/40 border border-purple-500/20 rounded-xl p-3">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[10px] font-semibold text-slate-300">Cole os dados do{' '}
              <a href="https://pt.semrush.com/ai-seo/overview/" target="_blank" rel="noopener noreferrer" className="text-brand-500 underline">SEMrush AI SEO</a>
            </p>
            <button onClick={() => setEditing(false)}><X size={12} className="text-slate-500" /></button>
          </div>
          <div className="grid grid-cols-2 gap-1.5 mb-2">
            {[{k:'score',l:'Score (0-100)',p:'34'},{k:'mentions',l:'Mencoes',p:'73'},{k:'citations',l:'Citacoes',p:'469'},{k:'pages',l:'Pags. citadas',p:'203'}].map(f => (
              <div key={f.k}><label className="block text-[10px] text-slate-500 mb-0.5">{f.l}</label>
                <input type="number" placeholder={f.p} value={draft[f.k]||''} onChange={e => setDraft(d=>({...d,[f.k]:e.target.value}))}
                  className="w-full bg-surface-border rounded px-2 py-1 text-xs text-white focus:outline-none focus:ring-1 focus:ring-purple-500" /></div>
            ))}
          </div>
          <p className="text-[9px] text-slate-600 mb-1.5">Distribuicao LLM %</p>
          <div className="grid grid-cols-2 gap-1.5 mb-2">
            {LLM_FIELDS.map(f => (
              <div key={f.k}><label className="block text-[10px] text-slate-500 mb-0.5">{f.l} %</label>
                <input type="number" step="0.1" value={draft[f.k]||''} onChange={e => setDraft(d=>({...d,[f.k]:e.target.value}))}
                  className="w-full bg-surface-border rounded px-2 py-1 text-xs text-white focus:outline-none focus:ring-1 focus:ring-purple-500" /></div>
            ))}
          </div>
          <button onClick={save} className="w-full bg-purple-500 hover:bg-purple-600 text-white text-xs font-semibold rounded-lg py-1.5 flex items-center justify-center gap-1.5 transition-colors">
            <Save size={10} /> Salvar
          </button>
        </div>
      )}
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          <p className="text-[10px] font-medium text-slate-500 uppercase tracking-wider flex items-center gap-1"><Bot size={9} /> LLM Visibility</p>
          {llm && (llm.score || llm.mentions) ? (
            <>
              ...
              <div className="grid grid-cols-3 gap-1">
                {[{l:'Mencoes',v:llm.mentions},{l:'Citacoes',v:llm.citations},{l:'Pags.',v:llm.pages}].map(m=>(
                  <div key={m.l} className="bg-surface-border/30 rounded-lg p-1.5 text-center">
                    <p className="text-[9px] text-slate-500">{m.l}</p><p className="text-sm font-bold text-purple-400">{m.v || '-'}</p>
                  </div>
                ))}
              </div>
              {llmBars.length > 0 && (
                  ...
                  {llmBars.map(l => (
                    <div key={l.k} className="flex items-center gap-1.5">
                      ...
                      <div className="flex-1 h-1 bg-surface-border rounded-full overflow-hidden">
                        <div className={'h-full ' + l.color + ' rounded-full opacity-80'} style={{width: Math.min(Number(l.v)||0,100) + '%'}} />
                      </div>
              ...
              {llm.syncedAt && <p className="text-[9px] text-slate-600">Sync: {new Date(llm.syncedAt).toLocaleDateString('pt-BR')}</p>}
            </>
          ) : (
            <div className="bg-purple-500/5 rounded-xl p-3 text-center">
              <p className="text-[10px] text-slate-500 mb-2">Dados do SEMrush AI SEO requerem sync manual</p>
              <button onClick={openEdit} className="inline-flex items-center gap-1 bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 text-[10px] font-medium rounded-lg px-2.5 py-1.5 transition-colors">
        ...
        <div className="space-y-2">
          <p className="text-[10px] font-medium text-slate-500 uppercase tracking-wider flex items-center gap-1"><Zap size={9} /> SERP Features (Google)</p>
          <div className="flex items-end gap-1.5">
          ...
          <div>
            ...
            <div className="h-1.5 bg-surface-border rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-emerald-500 to-emerald-400 rounded-full transition-all" style={{width: Math.min(score,100) + '%'}} />
            </div>
          </div>
          {topList.length > 0 && (
            <div className="space-y-1 max-h-28 overflow-y-auto">
              {topList.slice(0,6).map((k,i)=>(
                <div key={i} className="flex items-center gap-1.5 bg-surface-border/20 rounded px-1.5 py-1">
                  ...
                  <span className={'text-[9px] font-bold px-1 rounded ' + (k.position<=3?'text-emerald-400':'text-slate-400')}>#{k.position}</span>
                </div>
              ))}
            </div>
          )}
