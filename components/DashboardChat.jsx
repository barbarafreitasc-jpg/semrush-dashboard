...
export default function DashboardChat({ overview, keywords, competitors, backlinks }) {
  ...
  const abortRef = useRef(null); // cancela requisicoes anteriores em voo

  useEffect(() => {
    // Tenta carregar dados LLM do servidor primeiro, depois localStorage
    fetch('/api/llm-data', { cache: 'no-store' })
      .then(r => r.json())
      .then(d => { if (d) setLlmData(d); })
      .catch(() => {
        try {
  ...
  const buildContext = () => {
    ...
    if (backlinks?.overview) {
      ctx += 'Backlinks:\n- Total de backlinks: ' + fmt(backlinks.overview.total) + '\n- Dominios referenciando: ' + fmt(backlinks.overview.referringDomains) + '\n\n';
    }
    ...
    if (llmData) {
      ctx += 'GEO/IA: Score LLM: ' + llmData.score + '/100, Mencoes: ' + llmData.mentions + ', Citacoes: ' + llmData.citations + '\n';
      ctx += 'Google AI Overview: ' + llmData.googleaio + '%, Google AI Mode: ' + llmData.aimode + '%, Gemini: ' + llmData.gemini + '%, ChatGPT: ' + llmData.chatgpt + '%\n';
    }
    return ctx;
  };
  ...
  const handleAsk = async (q) => {
    ...
    // Cancela qualquer requisicao anterior ainda em voo
    if (abortRef.current) abortRef.current.abort();
    const controller = new AbortController();
    abortRef.current = controller;

    // Reset limpo antes de mostrar novo estado
    setSubmittedQ(pergunta);
    setAnswer('');
    setAsked(true);
    setLoading(true);
    ...
    try {
      const res = await fetch('/api/ask', {
        ...
        body: JSON.stringify({ question: pergunta, context: buildContext() }),
      ...
      if (err.name !== 'AbortError') {
        setAnswer('Erro ao processar. Tente novamente.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleReset = () => {
    // Cancela requisicao em voo se houver
      ...
      {!asked ? (
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-2 bg-gray-800/40 rounded-xl p-3 border border-gray-700/40">
            ...
            <p className="text-sm text-slate-300 flex-1">{submittedQ}</p>
          ...
          {!loading && (
            <button onClick={handleReset} className="text-[11px] text-slate-500 hover:text-violet-400 transition-colors">
              &larr; Nova pergunta
            </button>
          )}
        </div>
