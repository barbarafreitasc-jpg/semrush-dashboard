...
const fetcher = url => fetch(url).then(r => { if (!r.ok) return r.json().then(e => Promise.reject(e)); return r.json(); });
    ...
    <header className="border-b border-surface-border bg-surface-card/50 backdrop-blur-sm sticky top-0 z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {domain && (
            <button onClick={onRefresh} disabled={refreshing} title="Forcar atualizacao"
  ...
  return (
    <div className="bg-surface-card/60 border-b border-surface-border backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 flex items-center gap-3">
        ...
        <span className="text-xs text-slate-400 font-medium shrink-0">Periodo:</span>
        ...
        {value !== 'current' && (
          <span className="text-[10px] text-amber-500/70 hidden sm:block">
            KPIs de trafego e keywords refletem dados historicos do periodo
          </span>
        )}
      </div>
    </div>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-xl">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">Analise de SEO em tempo real via API do SEMrush</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-surface-card border border-surface-border rounded-2xl p-6 space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">Dominio para analise</label>
          ...
          <button type="submit" className="w-full bg-brand-500 hover:bg-brand-600 text-white font-semibold rounded-xl py-3 flex items-center justify-center gap-2 transition-colors">
            <Search size={15} /> Analisar dominio
          </button>
        </form>
      </div>
    </div>
...
function Dashboard({ domain, database, refreshInterval }) {
  ...
  const availableMonths = useMemo(() => {
    ...
    (history || []).forEach(h => {
      ...
      list.push({ value: h.date, label: d.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' }) });
    });
    return list;
  }, [history]);
  ...
  const errorMsg = criticalError?.error || criticalError?.message || 'Verifique sua API key e o dominio informado.';
  ...
  return (
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {/* INSIGHTS */}
        <InsightsBox overview={displayOverview} history={history} keywords={keywords} competitors={competitors}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KPICard title="Trafego Organico Est." value={formatNumber(displayOverview?.organicTraffic)} icon={<TrendingUp size={16} />} color="text-brand-500" loading={overviewLoading} subtitle="visitas/mes estimadas" />
          <KPICard title="Keywords Organicas" value={formatNumber(displayOverview?.organicKeywords)} icon={<Key size={16} />} color="text-emerald-400" loading={overviewLoading} subtitle="posicoes no top 100" />
          <KPICard title="Backlinks" value={formatNumber(backlinks?.overview?.total)} icon={<Globe size={16} />} color="text-purple-400" loading={blLoading} subtitle={`${formatNumber(backlinks?.overview?.referringDomains)} dominios ref.`} />
          <KPICard title="Concorrentes" value={formatNumber(competitors?.length)} icon={<Users size={16} />} color="text-orange-400" loading={compLoading} subtitle="detectados via API" />
        </div>
