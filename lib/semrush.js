 ...
 * @param {string|null} displayDate  Formato YYYYMM01 (ex: "20250101")
 */
export async function getDomainOverview(domain, database = 'br', displayDate = null) {
  const params = {
    type:           'domain_ranks',
    domain,
    database,
...
/**
 * Top palavras-chave orgânicas do domínio.
 * @param {string} domain
 * @param {string} database
 * @param {number} limit
 * @param {string|null} displayDate  Formato YYYYMM01
 */
export async function getOrganicKeywords(domain, database = 'br', limit = 50, displayDate = null) {
  const params = {
    type:           'domain_organic',
    domain,
    database,
 ...
 * @param {string|null} displayDate  Formato YYYYMM01
 */
export async function getCompetitors(domain, database = 'br', limit = 10, displayDate = null) {
  const params = {
...
/**
 * Visão geral de backlinks do domínio (sempre dado atual, sem filtro de data).
 */
...
/**
 * Dados de Visibilidade em IA (AI Overview / Google AI Overviews).
 * Busca top 100 keywords e identifica quais possuem AI Overview no Google.
 * A coluna 'Ai' indica presença de AI Overview (1 = sim, 0 = não).
 *
 * @param {string} domain
 * @param {string} database
 * @param {string|null} displayDate  Formato YYYYMM01
 */
export async function getAIVisibilityData(domain, database = 'br', displayDate = null) {
  const params = {
    type:           'domain_organic',
    domain,
    database,
  ...
  // Visibilidade calculada: % do tráfego do top-100 que vem de keywords com AI Overview
  const aiTrafficPct = totalTraffic > 0
    ? Math.round((aiTraffic / totalTraffic) * 100)
    : 0;

  // Score de visibilidade: % de keywords do top-100 com AI Overview
